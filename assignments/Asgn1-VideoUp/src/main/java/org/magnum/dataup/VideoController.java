...
public class VideoController {
	...
	@Autowired
	private CrudRepository<Video, Long> videos;
	...
	public @ResponseBody VideoStatus addVideoData(@PathVariable(VideoSvcApi.ID_PARAMETER) Long videoId, @RequestParam(VideoSvcApi.DATA_PARAMETER) MultipartFile videoData, HttpServletResponse response) throws IOException{
		Video video = this.videos.findOne(videoId);
		VideoStatus status = null;
		if(video == null) {
			response.sendError(HttpServletResponse.SC_NOT_FOUND, "That video does not exist, are you sure you have the right ID?");
		} else {
			try {
				VideoFileManager.get().saveVideoData(video, videoData.getInputStream());
				status = new VideoStatus(VideoStatus.VideoState.READY);
			} catch (IOException e) {
				response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Video data could not be saved.");
			}
		}
		return status;
	}
	...
	public @ResponseBody byte [] getVideoData(@PathVariable(VideoSvcApi.ID_PARAMETER) Long videoId, HttpServletResponse response) throws IOException{
		Video video = this.videos.findOne(videoId);
		...
		if(video == null) {
			...
			try {
				VideoFileManager.get().copyVideoData(video, data);
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
